...
func {{.initCode}}() (err error) {
    RedisPool, err = newRedisClients(&clientConf{
		Options: &redis.Options{
			Addr:     fmt.Sprintf("%s:%d", config.AppConf.RedisConfig.Ip, config.AppConf.RedisConfig.Port),
			Password: config.AppConf.RedisConfig.Pwd,
		},
		DB: config.AppConf.RedisConfig.Db,
	})
	return
}